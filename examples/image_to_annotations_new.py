...
pose_model = init_model(config_file, checkpoint_file, device='cuda:0')  # or device='cuda:0'
...
def image_to_annotations(img_fn: str, out_dir: str) -> None:
    ...
    # resp = requests.post("http://192.168.0.42:8080/predictions/drawn_humanoid_detector", files=request_data, verify=False)
    ...
    mask = get_segment(cropped)


    # # send cropped image to pose estimator
    # data_file = {'data': cv2.imencode('.png', cropped)[1].tobytes()}
    # resp = requests.post("http://192.168.0.42:8080/predictions/drawn_humanoid_pose_estimator", files=data_file, verify=False)
    # if resp is None or resp.status_code >= 300:
    #     raise Exception(f"Failed to get skeletons, please check if the 'docker_torchserve' is running and healthy, resp: {resp}")
    
    pose_results1 = inference_topdown(pose_model, img_fn)[0].to_dict()['pred_instances']['keypoints']
...
def get_segment(img : np.ndarray):
    net = build_model(hypar, device)
    image_tensor, orig_size = load_image(img, hypar) 
    return predict(net,image_tensor,orig_size, hypar, device)
